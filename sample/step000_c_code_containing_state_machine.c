...
ES_Event RunTemplateFSM(ES_Event ThisEvent)
{
    ...
    case Moving: // In light and moving 
    ...
    case Getting_Unstuck: //One of the bumper is triggered 
        if (ThisEvent.EventType == LIGHT_TO_DARK) 
        {
            stopMovement();
            nextState = Hiding;
        ...
        if (ThisEvent.EventType == ES_TIMEOUT && ThisEvent.EventParam == SIMPLE_SERVICE_TIMER)
        {
...
//        if (ThisEvent.EventType == BUMPER_RELEASED)
//        {
//            goForward(MOVING_SPEED);
//            nextState = Moving;
//            makeTransition = TRUE;

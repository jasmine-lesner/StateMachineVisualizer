...
<!--
  template generates graphviz digraph (see LIMITATIONS below)
-->
<xsl:template match="/">
  <xsl:text>
digraph fsm {
...
<xsl:template match="*[@op='=' and ./lvalue[@name='nextState'] and 1=count(ancestor::block_items[@class='Case'])]">
      ...
      <!-- locate first enclosing context that tests EventType (such as some if statement) -->
      <xsl:for-each select="
              ancestor::*[
                .//*[@op='==' and .//field[@name='EventType']]
              ][1]
      ">
          <!-- inside the first enclosing context that tests EventType take the value it is tested against -->
          <xsl:value-of select="
              .//*[@op='==' and .//field[@name='EventType']]
              /right/@name
            "/>
